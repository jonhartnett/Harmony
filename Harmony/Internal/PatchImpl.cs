...
namespace HarmonyLib
{
	...
	public static class PatchImpl
	{
		...
		private static void DefineImplProperty(Type instanceType, TypeBuilder typeBuilder, FieldInfo instance, string name, Type type)
		{
			...
			var field = AccessTools.DeclaredField(instanceType, name);
			if(field == null)
		...
		private static void DefineImplPatchEnumerableProperty(Type instanceType, TypeBuilder typeBuilder, FieldInfo instance, string name, MethodInfo factory, Type patchType)
		{
			...
			var wrapperFunc = typeof(Func<>).MakeGenericType(patchType, typeof(IPatch));

			//get Select
			var select = AccessTools.DeclaredMethod(typeof(Enumerable), "Select", new[]{ patchEnumerableType, wrapperFunc }, new []{ patchType, typeof(IPatch) });

			//implement property
			typeBuilder.DefineProperty(name, PropertyAttributes.None, typeof(IEnumerable<IPatch>), Type.EmptyTypes);

			//implement getter
			var getterBuilder = typeBuilder.DefineMethod($"get_{name}", MethodAttributes.Public | MethodAttributes.SpecialName | MethodAttributes.HideBySig, typeof(IPatch), Type.EmptyTypes);
			var il = getterBuilder.GetILGenerator();
			il.Emit(OpCodes.Ldarg_0);
			il.Emit(OpCodes.Ldfld, instance);
			il.Emit(OpCodes.Ldfld, field);
