...
{
	/// <summary>
	/// All independent Harmony instances share a common state which keeps track of patches.
	/// Shared state is setup in a dynamic assembly with a fixed name by the first Harmony that is loaded.
	/// All other Harmony instances create dynamic wrapper implementations of their interfaces over the types from the first Harmony.
	/// </summary>
	internal static class HarmonySharedState
	{
		private delegate IPatchInfo StateGetter(MethodBase method);
		private delegate IEnumerable<MethodBase> StateKeyGetter();

		static readonly string name = "HarmonySharedState";
		internal static readonly int internalVersion = 101;
		...
		/// </summary>
		private sealed class SharedPatchInfo : IPatchInfo
		{
		...
		/// </summary>
		internal sealed class SharedPatch : IPatch
		{
			...
			int IPatch.index => index;
			string IPatch.owner => owner;
			int IPatch.priority => priority;
			string[] IPatch.before => before;
			string[] IPatch.after => after;
			MethodInfo IPatch.patch => patch;
			...
			{
				return obj is IPatch other && patch == other.patch;
			}
		...
		static void CreateStateAccessors()
		{
			lock (name)
			{
				...
				DynamicMethod getImpl = new DynamicMethod("stateGet", typeof(IPatchInfo), new []{ typeof(MethodBase) }, typeof(HarmonySharedState).Module, true);
				...
				DynamicMethod getOrInsertImpl = new DynamicMethod("stateGetOrInsert", typeof(IPatchInfo), new []{ typeof(MethodBase) }, typeof(HarmonySharedState).Module, true);
		...
		internal static IPatchInfo GetPatchInfo(MethodBase method)
		{
			if(stateGet == null)
		...
		internal static IPatchInfo GetOrCreatePatchInfo(MethodBase method)
		{
